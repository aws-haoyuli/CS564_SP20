...
/**
 * @brief The central class which manages the buffer pool including frame allocation and deallocation to pages in the
 * file
 */
class BufMgr
{
private:
  ...
  /**
   * Array of BufDesc objects to hold information corresponding to every frame allocation from 'bufPool' (the buffer
   * pool)
   */
...
public:
   ...
   * @param page  	Reference to page pointer. Used to fetch the Page object in which requested page from file is read
   * in.
   */
   ...
   * All the frames assigned to the file need to be unpinned from buffer pool before this function can be successfully
   * called. Otherwise Error returned.
   *
   * @param file   	File object
