...
// -----------------------------------------------------------------------------
int testNum = 1;
const std::string relationName = "relB";
...
int intScan(BTreeIndex *index, int lowVal, Operator lowOp, int highVal,
            Operator highOp);
void indexTests();
void indexTestsWithoutRemove();
void intTestsWithSmallBuf();
void removeIndex();
void test1();
void test2();
void test3();
...
void test1() {
  // Create a relation with tuples valued 0 to relationSize
  // And call constructor twice, first to generate a blobfile
  // then open that blob file to test
  std::cout << "---------------------------------" << std::endl;
  std::cout << "Test btree with existing blobfile" << std::endl;
  createRelationForward();
  indexTestsWithoutRemove();
  deleteRelation();
}
...
void test2() {
  ...
  std::cout << "----------------------------------------------------"
            << std::endl;
  std::cout << "Test btree with existing blobfile on random relation"
            << std::endl;
...
void test3() {
  ...
  // then test b+tree with buffer manager initialized to 5 buffers
  std::cout << "------------------------------------" << std::endl;
  std::cout << "Test btree with small buffer manager" << std::endl;
  createRelationForward();
  intTestsWithSmallBuf();
  deleteRelation();
}
...
void indexTestsWithoutRemove() {
  if (testNum == 1) {
    ...
    BTreeIndex *tmp = new BTreeIndex(relationName, intIndexName, bufMgr,
    ...
    // run some tests
    checkPassFail(intScan(&index, 25, GT, 40, LT), 14) checkPassFail(
        intScan(&index, 20, GTE, 35, LTE), 16)
        checkPassFail(intScan(&index, -3, GT, 3, LT), 3) checkPassFail(
            ...
            checkPassFail(intScan(&index, 0, GT, 1, LT), 0) checkPassFail(
                ...
                checkPassFail(intScan(&index, 3000, GTE, 4000, LT), 1000)
                    checkPassFail(intScan(&index, 5000, GTE, 6000, LT), 1000)
                        checkPassFail(intScan(&index, 26, GTE, 26, LTE), 1)
                            checkPassFail(intScan(&index, 26, GTE, 26, LT), 0)
  }
  removeIndex();
}
...
void intTestsWithSmallBuf() {
  std::cout << "Create a B+ Tree index on the integer field with small buffer"
            << std::endl;
  BufMgr *smBufMgr = new BufMgr(6);
  BTreeIndex index(relationName, intIndexName, smBufMgr, offsetof(tuple, i),
                   INTEGER);
  checkPassFail(intScan(&index, 25, GT, 40, LT), 14)
      checkPassFail(intScan(&index, 20, GTE, 35, LTE), 16)
          checkPassFail(intScan(&index, -3, GT, 3, LT), 3) checkPassFail(
